...
(function(){
  ...
  var view = Backbone.View.extend({
    ...
    initialize: function(options){
      if(!this.el) return;
      this.configure();
      this.friends = options.collection || new friend_collection();
      this.friends.bind("reset", this.render, this);
      this.loading(true);
      this.friends.fetch();
      // if the selector is in a form, don't let it submit on enter
      this.$el.closest('form').keypress(function(e){
    ...
    render: function(){
      ...
      this.friend_list.on('mouseleave', _.bind(function(){
        this.$el.delegate("#fs_search", "blur", _.bind(this.blur, this));
        this.blur();
      },this));
    },
    ...
    handle_mouseleave_item: function(e){
      ...
      this.$el.delegate("blur", "#fs_search", _.bind(this.blur, this));
    },
